...
| 1.10 | CLI (`amber <url>` + output flags + `-o`/`-n`/`--render`/`--wait-for`/`--min-content`) | `amber <url> -o ./out --markdown --readable --screenshot` works, HTTP-only when possible | 1.5, 1.6, 1.7, 1.9 | cc:完了 |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 2.1 | MCP server (read/screenshot/extract tools) | An MCP client can request a capture and receive the output | Phase 1 | cc:完了 |
...
| 2.4 | Resource blocking (ads/trackers/optional images) | Blocked resource classes are not fetched; faster renders | 1.4 | cc:完了 |
| 2.5 | Agent-native action primitives (navigate/click/fill/scroll/wait) | Actions executable via core API and MCP | 1.4, 2.1 | cc:完了 |
...
| 3.3 | Auth & session (cookies, headers, storage-state) | A behind-auth page is captured given supplied session state | 1.4 | cc:完了 |
...
| 3.10 | Secrets handling for auth (never logged) | Auth secrets never appear in logs/traces | 3.3 | cc:完了 |
...
| 4.4 | Provenance map (fact → DOM node + screenshot region + URL) | Each extracted field carries a verifiable anchor | 4.1, 1.7 | cc:完了 |
...
| 5.3 | WARC emitter (network recording before navigate) | `--warc` writes a valid WARC | 1.4 | cc:完了 |
| 5.4 | WACZ emitter (replayable) | `--wacz` round-trips as a replayable archive | 5.3 | cc:完了 |
...
| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 6.1 | UniFFI bindings — Python first (maturin → PyPI) | `pip install amber-html` gives `amber.snapshot(url)` | Phase 1 | cc:完了 |
| 6.2 | C ABI (cbindgen) + long-tail wrappers | A C header + shared lib expose the core; one long-tail wrapper builds | 6.1 | cc:完了 |
| 6.3 | Node bindings (napi-rs) | `require('amber')` snapshots a URL | 6.1 | cc:完了 |
| 6.4 | Self-healing selectors | Extraction recovers when a selector drifts | 4.1 | cc:完了 |
| 6.5 | Chunking / summarization for downstream consumers | Output chunked with stable boundaries | 1.5 | cc:完了 |
| 6.6 | Pagination / infinite-scroll | Paginated content captured across pages | 2.7 | cc:完了 |
| 6.7 | Packaging (cargo / PyPI / npm / Homebrew / Docker / binaries) | Installable from each listed channel | 6.1, 6.2, 6.3 | cc:WIP |

## Phase 7: v0.7 — Scale & ops (P2)

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 7.1 | Browser/tab pooling & reuse | Concurrent captures reuse a bounded pool | Phase 1 | cc:完了 |
...
| 7.4 | Resource limits | Per-capture memory/CPU/time caps enforced | 7.1 | cc:完了 |
| 7.5 | Local semantic memory (embeddings, offline search) | Captured corpus is searchable offline | 3.5 | cc:完了 |
